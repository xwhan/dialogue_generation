...
class Seq2SeqAttn(object):
	...
	def __init__(self, vocab_size, embed_size, num_layers, hidden_size, initial_embed=None):
		...
		self.decoder_tragets_length = tf.placeholder(shape=(None,), dtype=tf.int32, name='decoder_targets_length')
		...
		for _ in range(num_layers):
			...
			cells_1.append(tf.contrib.rnn.LSTMCell(hidden_size))

		self.encoder_cell = MultiRNNCell(cells)
		self.decoder_cell = MultiRNNCell(cells_1)
		...
		with tf.variable_scope('decoder') as scope:
			...
			attention_state = tf.transpose(self.encoder_outputs, [1, 0, 2])
